...
function check_reg(reg, insertText, message){//(정규식, id, 메세지)
	//test():입력한 값(insertText.value)이 정규식(reg)에 맞게 입력했으면 true를 반환
	if(reg.test(insertText.value)){
		return true;
	}
	...
	//방법-1
	//insertText.value = "";
	//return insertText.focus();
	
	//방법-2
	//return insertText.select();
...
function check() {
	//let me_id = document.getElementById("me_id");
	...
	//회원 아이디 유효성 검사 - check_reg() 함수 이용	
	if(!f.me_id.value){//if(f.me_id.value==""){
		alert("회원아이디를 입력해주세요.");
		return f.me_id.focus();		
	}
	else if(!check_reg(regIDPass, f.me_id, "아이디는 3~12자의 영어 대소문자와 숫자로만 입력 가능합니다.") ){
	...
	}else if(!check_reg(regIDPass, f.me_pass, "비밀번호는 3~12자의 영어 대소문자와 숫자로만 입력 가능합니다.") ){
	...
	}else if( !check_reg(regName, f.me_name, "이름이 잘못 입력되었습니다.") ){//이름에 특수문자가 들어간 경우
	...
	if(f.tel.value==""){
		alert("전화번호를 입력해주세요.");
		return f.tel.focus();		
	}else if( !check_reg(regTel, f.tel, "전화번호가 잘못 입력되었습니다.")){
