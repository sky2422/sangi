...
<body>
	...
	//★★trainer_tbl에서 tname과 ttime을 가져다 표시
				...
				<tr>
					...
					<td>
						<select name="ttime">
							...
							<%		
							sql = "select NVL(ttime,'0') AS ttime from trainer_tbl";
							rs = stmt.executeQuery(sql);
							
							if(!rs.next() && !rs.getString("ttime").equals("0")){			
								//if(!rs.getString("ttime").equals("0")){
									TreeSet<String> allTime = new TreeSet<String>();
									
									String ttime = 	null;
									
									while(rs.next()){								
										allTime.add(rs.getString("ttime"));								
										System.out.println(rs.getString("ttime"));//콘솔에 출력								
									}	
									System.out.println();//구분위한 빈줄 
									
									Iterator itr= allTime.iterator();							
									while(itr.hasNext()) {
								...
								//System.out.print("등록된 트레이너 선택시간이 없습니다.");//콘솔에 출력
								%>
									<script>
										alert("등록된 트레이너 선택시간이 없습니다.");
									</script>
								<%								
							} %>
